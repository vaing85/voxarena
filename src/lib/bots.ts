...
export type BotPreset = "rookie" | "pro" | "metro" | "soul";
...
function difficultyFactor(difficulty: string): number {
  const d = difficulty.toLowerCase();
  if (d === "hard") return 1.08;
...
): LayerScores & { scoreTotal: number } {
  const p = PRESETS[preset];
  const df = difficultyFactor(songDifficulty);

  const r = (n: number) => hash01(seed, n);
  const skip = r(99) < p.skipChance;

  let scorePitch = Math.round(
    (p.pitchBase + (r(1) - 0.5) * 2 * p.pitchVariance) * df
  );
  if (skip) scorePitch = Math.max(40, scorePitch - 25);

  let scoreTiming = Math.round(
    (72 + (r(2) - 0.5) * p.timingJitter * 2) * df
  );
  let scoreStability = Math.round(
    (70 + (r(3) - 0.5) * p.stabilityNoise * 2) * df
  );
...
export function listBotPresets(): BotPreset[] {
  return Object.keys(PRESETS) as BotPreset[];
}
