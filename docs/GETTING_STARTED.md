...
| POST | `/performances/audio` | 🔒 Multipart `audio` (WAV) + `playerId`, `songId`, `mode`. Real pitch, **timing & stability** via the pitch service. Needs `PITCH_SERVICE_URL` + song reference notes. |
