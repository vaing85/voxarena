...
| **Song-content pipeline** | Done: `parseMelody` converts compact notation (`NOTE:DUR`) → `Song.referenceNotes`; `npm run melody` author CLI; seed now adds public-domain singable songs (Twinkle, Mary Had a Little Lamb, Ode to Joy) with real reference melodies + lead-in. MIDI import can target the same note-list output. |
