...
exports.botDuelRouter = botDuelRouter;
const express_1 = require("express");
const ids_js_1 = require("../lib/ids.js");
const config_js_1 = require("../config.js");
const bots_js_1 = require("../lib/bots.js");
const stubScore_js_1 = require("../lib/stubScore.js");
function botDuelRouter(prisma) {
    const r = (0, express_1.Router)();
    r.get("/presets", (_req, res) => {
        res.json({ presets: (0, bots_js_1.listBotPresets)() });
    });
    /**
    ...
    r.post("/solo-vs-bot", async (req, res) => {
        ...
        if (!(0, bots_js_1.listBotPresets)().includes(botPreset)) {
            res.status(400).json({
                error: `botPreset must be one of: ${(0, bots_js_1.listBotPresets)().join(", ")}`,
            });
            return;
        }
        const [human, song, houseBot] = await Promise.all([
            ...
            prisma.player.findFirst({ where: { deviceId: config_js_1.HOUSE_BOT_DEVICE_ID } }),
        ]);
        if (!human || !song) {
        ...
        const hasAnyScore = req.body.scoreTotal != null || req.body.scorePitch != null;
        let humanLayers;
        if (!hasAnyScore) {
            humanLayers = (0, stubScore_js_1.computeStubScores)(seed);
        }
        else {
            ...
            if (pitch == null ||
                ...
                total == null) {
                res.status(400).json({
                    error: "Send all score fields or omit all for stub scoring",
                });
                return;
            }
            humanLayers = {
        ...
        const botLayers = (0, bots_js_1.generateBotScores)(botPreset, seed, song.difficulty);
